...
then
    ...
    find / \( -path /proc -o -path /sys -o -path /opt -o -path /build \) -prune -o -group "$SC_USER_ID" -exec chgrp --no-dereference "$CONT_GROUPNAME" {} \;
    # change user property of files already around
    echo "Changing ownership properties of files around from $SC_USER_ID to group $CONT_GROUPNAME"
    find / \( -path /proc -o -path /sys -o -path /opt  -o -path /build \) -prune -o -user "$SC_USER_ID" -exec chown --no-dereference "$SC_USER_NAME" {} \;
fi
